...
with st.sidebar:
    ...
    rs_interval = "1wk" if interval == "1wk" else "1d"
...
df_rs = pd.DataFrame()
if show_rs:
    bench_df = get_stock_data(rs_bench, period=period, interval=rs_interval)
    tick_df  = get_stock_data(ticker,   period=period, interval=rs_interval)
...
if rs_state_label and rs_state_label != "Insufficient Data":
    ...
    k6.metric("RS State",
              f"{cfg.get('emoji','')} {rs_state_label}",
              f"{rs_ext:+.1f}% ext")
else:
...
# ── Build subplots ─────────────────────────────────────────────────────────────
subplot_flags = [show_vol, show_rsi, show_macd, show_adx, show_stoch, (show_rs and not df_rs.empty)]
...
if show_bb:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df["BB_Mid"],
                             ...
                             showlegend=False, name="BB Mid"), row=1, col=1)
...
for subplot_name, _ in active:
    ...
    if subplot_name == "Volume":
    ...
    elif "RS vs" in subplot_name and not df_rs.empty:
        # RS ratio line
        ...
        for col_name, color, label in [
            ...
            fig.add_trace(go.Scatter(x=df_rs.index, y=df_rs[col_name], name=label,
                                     line=dict(color=color, width=1.4)), row=r, col=1)
        # Extension bands
        for band_col, color, label in [
            ("ext5",  "#4fc3f7", "+5%"),
            ("ext10", "#ef5350", "+10%"),
            ("ext15", "#ff8c00", "+15%"),
            ("ext20", "#ffd700", "+20%"),
        ]:
            fig.add_trace(go.Scatter(x=df_rs.index, y=df_rs[band_col], name=label,
                                     line=dict(color=color, width=1, dash="dot"),
                                     opacity=0.5), row=r, col=1)
        # RS State annotation
        if rs_state_label:
            cfg_ = STATE_CONFIG.get(rs_state_label, {})
            fig.add_annotation(
                xref="paper", yref=f"y{r}",
                x=1.0, y=df_rs["RS"].iloc[-1],
                text=f" {cfg_.get('emoji','')} {rs_state_label}",
                showarrow=False, font=dict(color=cfg_.get("color","white"), size=11),
...
with tab1:
    ...
    r_val = last["RSI"]
    if r_val > 70:
        signals.append(("⚠️", f"RSI {r_val:.1f} — Overbought", "Caution"))
    elif r_val < 30:
        signals.append(("⚠️", f"RSI {r_val:.1f} — Oversold", "Caution"))
    else:
    ...
    if last["ADX"] > 25:
        ...
        signals.append(("💪", f"ADX {last['ADX']:.1f} — Strong {td}", td))
    else:
        signals.append(("😴", f"ADX {last['ADX']:.1f} — Weak / ranging", "Ranging"))

    if last["BB_PctB"] > 1.0:
        signals.append(("📊", "Price above upper Bollinger Band", "Caution"))
    elif last["BB_PctB"] < 0.0:
        signals.append(("📊", "Price below lower Bollinger Band", "Caution"))

    st.dataframe(
        pd.DataFrame(signals, columns=["", "Signal", "Bias"]),
...
with tab2:
    if rs_state_label and rs_state_label != "Insufficient Data":
        ...
        st.info(
            f"📖 **{rs_state_label}:** {cfg_['description']}  \n"
            f"➡ **Suggested Action:** {cfg_['action']}"
        )
        ...
        rs_signals = [
            ...
            ("SMA18 Direction", "Rising 📈" if df_rs["SMA18"].iloc[-1] > df_rs["SMA18"].iloc[-2] else "Falling 📉"),
            ("Extension", f"{rs_ext:+.1f}%"),
        ]
        st.dataframe(
            pd.DataFrame(rs_signals, columns=["Condition", "Status"]),
            width='stretch', hide_index=True
        )
    else:
        st.info(f"RS data unavailable — need 40+ bars. Try a longer period (1y+) with weekly interval.")

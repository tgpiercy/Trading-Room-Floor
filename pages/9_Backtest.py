...
# Diagnostics live only in the updated backtest.py. If an older copy is deployed
# (page pushed before the util), the page still loads — the diagnostics section
# just shows an "update backtest.py" note instead of hard-crashing.
try:
    from utils.backtest import (multi_horizon_table, concentration_check,
                                subperiod_split)
    _DIAG = True
except ImportError:
    _DIAG = False
...
if not _DIAG:
    st.warning("⚠️ Diagnostics need the updated **utils/backtest.py** (the deployed copy "
               "is the older version). Push utils/backtest.py to enable the horizon, "
               "concentration, and regime checks. Phase 1 results above are unaffected.")
    st.stop()
